...
def make_request(**overrides) -> PlanRequest:
    base = dict(
        ...
        sanitizedPageStructure=[
            SanitizedNode(tag="input", control="CONTROL_1", inputType="email", label="Email", filled=False, disabled=False),
            SanitizedNode(tag="input", control="CONTROL_2", inputType="tel", label="Phone", filled=False, disabled=False),
            SanitizedNode(tag="button", control="CONTROL_3", label="Submit", filled=False, disabled=False),
        ],
...
def test_plan_advances_to_phone_then_submit():
    filled_email = make_request(
        sanitizedPageStructure=[
            SanitizedNode(tag="input", control="CONTROL_1", inputType="email", label="Email", filled=True, disabled=False),
            SanitizedNode(tag="input", control="CONTROL_2", inputType="tel", label="Phone", filled=False, disabled=False),
            SanitizedNode(tag="button", control="CONTROL_3", label="Submit", filled=False, disabled=False),
        ]
    )
    ...
    filled_both = make_request(
        sanitizedPageStructure=[
            SanitizedNode(tag="input", control="CONTROL_1", inputType="email", label="Email", filled=True, disabled=False),
            SanitizedNode(tag="input", control="CONTROL_2", inputType="tel", label="Phone", filled=True, disabled=False),
            SanitizedNode(tag="button", control="CONTROL_3", label="Submit", filled=False, disabled=False),
        ]
    )
...
def test_plan_schema_rejects_raw_selector_fields_and_nonopaque_controls():
    payload = make_request().model_dump()
    payload["sanitizedPageStructure"][0]["selector"] = '[name="alice@example.test"]'
    response = client.post("/v1/plan", json=payload)
    assert response.status_code == 422
    assert "alice@example.test" not in response.text

    payload = make_request().model_dump()
    payload["sanitizedPageStructure"][0]["control"] = "#email"
    assert client.post("/v1/plan", json=payload).status_code == 422
