...
| **R11** | onnxruntime-web WASM threads/SIMD need **COOP/COEP**; models must be **bundled locally** | **Resolved by removal (M3 model gate):** no ONNX model and no `onnxruntime-web` ship. The evaluated detector was rejected on measured uselessness for browser UI (`docs/m3-visual-perception.md` §9), so the COOP/COEP question is moot. The bundled OCR engine (Tesseract.js) runs with `numThreads` unset and `workerBlobURL: false` under MV3 CSP. If any model is ever added it MUST be a bundled extension asset — never fetched at runtime in the privacy path |
